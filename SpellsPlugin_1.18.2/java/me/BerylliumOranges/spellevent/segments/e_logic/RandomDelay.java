...
public class RandomDelay extends EventSegment {
	...
	public static final double DELAY_LENGTH = 0.1;
	...
	public static Guideline processSegment(SegmentProcess proc) {
		...
		}, (long) (Math.random() * getDuration(upgrades) * 20));
	...
	public static ItemStack getUpdatedItem(int upgrades[], int dir) {
		...
		meta.setDisplayName(getColorFromRarity(RARITY) + "Random Delay");
		meta.setLore(Arrays.asList(ChatColor.WHITE + "Delays following segments ",
				ChatColor.WHITE + "for up to " + ChatColor.GRAY + getDuration(upgrades) + ChatColor.WHITE + " seconds "
						+ ChatColor.GRAY + SpellActions.getArrowModelFromInt(dir),
				ChatColor.DARK_GRAY + "For dramatic effect...", getCostText(getDuration(upgrades))));
	...
	public static double getDuration(int upgrades[]) {
		return DELAY_LENGTH + (upgrades[0] + upgrades[1]) / 50.0;
	}
}
