...
public class Delay extends EventSegment {
	...
	public static final double DELAY_LENGTH = 0.5;
	...
	public static ItemStack getUpdatedItem(int upgrades[], int dir) {
		...
		meta.setDisplayName(getColorFromRarity(RARITY) + "Delay Segment");
		meta.setLore(Arrays.asList(ChatColor.WHITE + "Delays following segments ",
				ChatColor.WHITE + "by " + ChatColor.GRAY + getDuration(upgrades) + ChatColor.WHITE + " seconds "
						+ ChatColor.GRAY + SpellActions.getArrowModelFromInt(dir),
				ChatColor.DARK_GRAY + "Does not crit", getCostText(getDuration(upgrades))));
	...
	public static double getDuration(int upgrades[]) {
		return DELAY_LENGTH + (upgrades[0] + upgrades[1]) / 2.0;
	}
}
